...
# FID assesses 保真度，the smaller the better
def calculate_fid(
    ...
    if not np.isfinite(covmean).all():
        print(
            "WARNING: fid calculation produces singular product; "
            f"adding {eps} to the covariance diagonal"
        )
    ...
    return {"frechet_distance": float(fid)}

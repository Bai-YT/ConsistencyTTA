...
def style_transfer(
    ...
    assert get_bit_depth(original_audio_file_path) == 16, \
        "The bit depth of the original audio file %s must be 16" % original_audio_file_path
    ...
    if duration >= audio_file_duration:
        print(f"Warning: Duration you specified {duration}-seconds must equal or smaller "
              f"than the audio file duration {audio_file_duration}s")
...
def super_resolution_and_inpainting(
    ...
    with torch.no_grad():
        waveform = latent_diffusion.generate_sample_masked(
            [batch], unconditional_guidance_scale=guidance_scale,
            ddim_steps=ddim_steps, duration=duration,
            n_candidate_gen_per_text=n_candidate_gen_per_text,

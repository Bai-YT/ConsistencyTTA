...
class DDIMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def set_timesteps(self, num_inference_steps: int, device: Union[str, torch.device] = None):
        ...
        self.timesteps = torch.from_numpy(timesteps.astype(np.int64)).to(device)
        # self.timesteps += self.config.steps_offset
        # print(f"avail_timesteps: {self.timesteps}")
    ...
    def step(
        ...
        prev_timestep = timestep.cpu() - self.config.num_train_timesteps // self.num_inference_steps
        ...
        if self.config.prediction_type == "epsilon":
            ...
            sqrt_alpha_prod = (alpha_prod_t ** 0.5).reshape(-1, 1, 1, 1).to(sample.device)
            sqer_beta_prod = (beta_prod_t ** 0.5).reshape(-1, 1, 1, 1).to(model_output.device)
            pred_original_sample = sqrt_alpha_prod * sample - sqer_beta_prod * model_output
        ...
        variance = self._get_variance(timestep.cpu(), prev_timestep).reshape(-1, 1, 1, 1)
        ...
        pred_sample_direction = (1 - alpha_prod_t_prev - std_dev_t ** 2).to(pred_epsilon.device) ** 0.5 * pred_epsilon

        # 7. compute x_t without "random noise" of formula (12) from https://arxiv.org/pdf/2010.02502.pdf
        prev_sample = alpha_prod_t_prev.to(pred_epsilon.device) ** (0.5) * pred_original_sample + pred_sample_direction

...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        ...
        patch_stride = 16, enable_fusion=False, fusion_type='None'
    ):
        super().__init__()
        ...
        if (self.enable_fusion) and (self.fusion_type in ['daf_2d','aff_2d','iaff_2d']):
            self.mel_conv2d = nn.Conv2d(
                in_chans, embed_dim, kernel_size=(patch_size[0], patch_size[1]*3), 
        ...
        if (self.enable_fusion) and (self.fusion_type in ['daf_2d','aff_2d','iaff_2d']):
            ...
            if len(longer_idx) > 0:
                ...
                if local_x.size(-1) < TW:
                    local_x = torch.cat([
                        local_x, torch.zeros((TB,TC,TH,TW-local_x.size(-1)), device=global_x.device)
                    ], dim=-1)
                else:
                    local_x = local_x[:,:,:,:TW]
...
class HTSAT_Swin_Transformer(nn.Module):
    ...
    def __init__(self, spec_size=256, patch_size=4, patch_stride=(4,4), 
        ...
        self.logmel_extractor = LogmelFilterBank(
            ...
            fmin=config.fmin, fmax=config.fmax, ref=ref, amin=amin, top_db=top_db, 
    ...
    def reshape_wav2img(self, x):
        ...
        assert T <= target_T, \
            f"the wav size should less than or equal to the swin input size. T={T}, target_T={target_T}"
        assert F <= target_F, \
            f"the wav size should less than or equal to the swin input size. F={F}, target_F={target_F}"

...
class DDPMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def __init__(
        ...
        print("Using the DDPM scheduler.")
    ...
    def set_timesteps(
        ...
        if num_inference_steps > self.config.num_train_timesteps:
            raise ValueError(
                f"`num_inference_steps`: {num_inference_steps} cannot be "
                "larger than `self.config.train_timesteps`: "
                f"{self.config.num_train_timesteps} as the unet model "
                "trained with this scheduler can only handle "
    ...
    def _get_variance(self, t, predicted_variance=None, variance_type=None):
        num_inference_steps = (
            self.num_inference_steps if self.num_inference_steps
    ...
    def step(
        ...
        if (model_output.shape[1] == sample.shape[1] * 2 
            and self.variance_type in ["learned", "learned_range"]
        ):
        ...
        if self.config.prediction_type == "epsilon":
            ...
            pred_original_sample = (
                (alpha_prod_t ** 0.5).reshape(-1, 1, 1, 1) * sample -
                (beta_prod_t ** 0.5).reshape(-1, 1, 1, 1) * model_output
            )
        else:
            raise ValueError(
                f"prediction_type given as {self.config.prediction_type} must be one of `epsilon`,"
                " `sample` or `v_prediction` for the DDPMScheduler."
            )
        ...
        # See formula (7) from https://arxiv.org/pdf/2006.11239.pdf
        pred_prev_sample = (
            pred_original_sample_coeff.reshape(-1, 1, 1, 1) * pred_original_sample +
            current_sample_coeff.reshape(-1, 1, 1, 1) * sample
        )
        ...
        variance_noise = randn_tensor(
            model_output_ind.shape, generator=generator,
            device=device, dtype=model_output_ind.dtype
        )

        if self.variance_type == "fixed_small_log":
            variance_ind = self._get_variance(
                t_ind, predicted_variance=predicted_variance_ind
            ) * variance_noise
        elif self.variance_type == "learned_range":
            variance_ind = self._get_variance(t_ind, predicted_variance=predicted_variance_ind)
            variance_ind = torch.exp(0.5 * variance_ind) * variance_noise
        else:
            variance_ind = (
                self._get_variance(t_ind, predicted_variance=predicted_variance_ind) ** 0.5
            ).reshape(-1, 1, 1, 1) * variance_noise
